...
module.exports = Backbone.View.extend({
	...
	showRoot: function(){
		...
		self.vRibbon.bind('onScrollAnchor', function($anchor){
			...
			if ($anchor.data('anchor') == 'headline'){
				...
				self.vSimpleCellAuto.gamma = 0;
				self.vSimpleCellAuto.initEmptyEnv(5,5);
				self.vSimpleCellAuto.initIndividuals(3,3,3,3);
				self.vSimpleCellAuto.startMovement(1000);

			}else if ($anchor.data('anchor') == 'cellauto-sim'){

				var n = 80;
				//var m = Math.round(n*2/Math.sqrt(3));
				var m = n;
				if (m%2 > 0){
					m++;
				}
				...
				var R0 = (2.4 + 3.3)/2;
				//var R0 = 5;
				var incDur = 5; // Inkubationszeit
				var illDur = 3; // mittlere Dauer des Krankheitsverlaufes
				var D = incDur + illDur;
				var meanContact = P/(n*m);
				var infectProb = R0 / (meanContact*D);

				console.log(meanContact);
				console.log(infectProb);

				self.vSimpleCellAuto.showPlot = true;
				self.vSimpleCellAuto.showInfection = true;
				self.vSimpleCellAuto.show();
				self.vSimpleCellAuto.alpha = infectProb;
				self.vSimpleCellAuto.beta = 1/illDur;
				self.vSimpleCellAuto.gamma = 1/incDur;

				self.vSimpleCellAuto.alpha = 0.1;
				self.vSimpleCellAuto.beta = 1/3;
				self.vSimpleCellAuto.gamma = 1/10;

				self.vSimpleCellAuto.simDuration = 200;
				self.vSimpleCellAuto.initEmptyEnv(n,m);
				self.vSimpleCellAuto.initIndividuals(P,0,0,0);
				self.vSimpleCellAuto.initIndividualsInRange(n/2,m/2,3,1,2);
	...
	initBackDiagrams: function(){
		...
		var sirSim = new MSim();
		sirSim.set('simulationDuration', 100);
		sirSim.set('initValues', [1, 0.99, 0.01, 0]);
		sirSim.set('params', [
